#!/usr/bin/env python3
"""Immutable one-click bootstrap for Dialforge Final Acceptance v3 on Colab."""
...
RUNNER = ROOT / "dialforge_final_acceptance_v3.py"
REPORT_DIR = ROOT / "dialforge-final-acceptance-v3"
REPORT_HTML = REPORT_DIR / "dialforge-final-acceptance-v3.html"
REPORT_JSON = REPORT_DIR / "dialforge-final-acceptance-v3.json"
REPORT_ZIP = ROOT / "dialforge-final-acceptance-v3-results.zip"
OWNER = "SumamaAhmed69"
REPO = "Axemetric-Caller-Beta-Runtime"
RUNNER_PATH = "benchmarks/dialforge_final_acceptance_v3.py"
...
def run(cmd, *, check=True, capture=False, env=None):
    ...
    if check and result.returncode != 0:
        ...
        if capture and result.stdout:
            detail = "\n--- command output ---\n" + result.stdout[-20000:]
        raise RuntimeError(f"Command failed ({result.returncode}): {shown(cmd)}{detail}")
    return result
...
def install_ollama():
    if ollama_works():
        return
    print("Installing Ollama from the official resumable Linux archive...", flush=True)
...
def setup_python_stack(python: pathlib.Path):
    ...
    run([
        str(python), "-m", "pip", "install", "torch==2.6.0", "torchaudio==2.6.0",
        "--index-url", "https://download.pytorch.org/whl/cu124",
    ], env=env)
    run([
        str(python), "-m", "pip", "install", "numpy==1.26.4", "psutil==7.0.0", "requests==2.32.5",
...
def validate_stack(python: pathlib.Path):
    print("\n=== Validating Dialforge v3 AI stack ===", flush=True)
    checks = [
        ("Python", "import sys; print(sys.version); assert sys.version_info[:2]==(3,11)"),
        ("NumPy", "import numpy as n; print(n.__version__); assert n.__version__=='1.26.4'"),
...
def start_ollama():
    ...
    env = os.environ.copy()
    env.update({
        "OLLAMA_HOST":"127.0.0.1:11434", "OLLAMA_ORIGINS":"*", "OLLAMA_KEEP_ALIVE":"30m",
        "OLLAMA_MAX_LOADED_MODELS":"1", "OLLAMA_NUM_PARALLEL":"1", "OLLAMA_FLASH_ATTENTION":"1",
    })
    log_handle = open(OLLAMA_LOG, "w")
    process = subprocess.Popen([shutil.which("ollama") or "ollama", "serve"], stdout=log_handle, stderr=subprocess.STDOUT, env=env)
    for _ in range(90):
        if ollama_ready():
            print("Ollama server ready with production benchmark settings.", flush=True)
            return
...
def api_json(url: str):
    req = urllib.request.Request(url, headers={"User-Agent":"Dialforge-Final-v3","Accept":"application/vnd.github+json"})
...
def download_runner():
    sha = source_sha()
    print(f"Downloading tool-aware runner from immutable commit {sha[:12]}...", flush=True)
    ...
    if item.get("encoding") != "base64" or not item.get("content"):
        raise RuntimeError("GitHub did not return the v3 runner content.")
...
def run_benchmark(python: pathlib.Path):
    ...
    ], check=False, env=benchmark_env())
    if not REPORT_HTML.exists() or not REPORT_JSON.exists():
        raise RuntimeError(f"V3 acceptance exited with {result.returncode} without complete report files.")
    if REPORT_ZIP.exists():
        REPORT_ZIP.unlink()
    shutil.make_archive(str(REPORT_ZIP.with_suffix("")), "zip", REPORT_DIR)
...
def main():
    ...
    print("=== DIALFORGE FINAL COLAB ACCEPTANCE BOOTSTRAP v3 ===", flush=True)
    ...
    if not args.no_require_cuda:
        if not shutil.which("nvidia-smi"):
            raise RuntimeError("No NVIDIA GPU. In Colab choose Runtime > Change runtime type > T4 GPU.")
        run(["nvidia-smi"])
    apt_setup()
    install_ollama()
    python = ensure_py311()
    ...
    if not args.no_require_cuda:
        validate_stack(python)
    start_ollama()
    download_runner()
    if args.validate_only:
        print("Final v3 bootstrap validation passed.", flush=True)
        return 0
    code = run_benchmark(python)
    print("\n=== DIALFORGE FINAL ACCEPTANCE v3 COMPLETE ===", flush=True)
    print("HTML:", REPORT_HTML, flush=True)
    print("JSON:", REPORT_JSON, flush=True)
    print("ZIP :", REPORT_ZIP, flush=True)
    print("Runner exit code:", code, flush=True)
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print("\nFinal v3 acceptance interrupted by user.", flush=True)
        raise
    except Exception as exc:
        print("\n=== FINAL v3 ACCEPTANCE STOPPED SAFELY ===", flush=True)
        print(f"{type(exc).__name__}: {exc}", flush=True)
        print("Everything happened inside the temporary Colab VM; nothing was installed on your PC.", flush=True)
        raise SystemExit(1)
