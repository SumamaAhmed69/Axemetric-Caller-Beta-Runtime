#!/usr/bin/env python3
"""One-click immutable bootstrap for Dialforge Final Acceptance on Google Colab.

The notebook pins this bootstrap and the benchmark runner to the same Git commit. The
bootstrap provisions Python 3.11, CUDA-compatible Dialforge speech dependencies and a
resilient Ollama install, then creates the final HTML/JSON report plus a ZIP bundle.
"""
...
RUNNER = ROOT / "dialforge_final_acceptance.py"
...
RUNNER_PATH = "benchmarks/dialforge_final_acceptance.py"
...
def download_with_resume(url: str, destination: pathlib.Path, attempts: int = 12):
    ...
    for attempt in range(1, attempts + 1):
        ...
        result = run([
            ...
            "--retry", "5", "--retry-all-errors", "--retry-delay", "2", "-C", "-",
...
def setup_python_stack(python: pathlib.Path):
    ...
    run([
        ...
        "numpy==1.26.4", "psutil==7.0.0", "requests==2.32.5",
...
def validate_stack(python: pathlib.Path):
    print("\n=== Validating final Dialforge AI stack ===", flush=True)
    checks = [
...
def download_runner():
    sha = source_sha()
    print(f"Downloading final runner from immutable commit {sha[:12]}...", flush=True)
    ...
    if item.get("encoding") != "base64" or not item.get("content"):
        raise RuntimeError("GitHub did not return the final runner content.")
...
def run_benchmark(python: pathlib.Path):
    ...
    result = run([
        ...
        "--comparison-repeats", "4",
        "--output-dir", str(REPORT_DIR),
    ], check=False, env=benchmark_env())
    if not REPORT_HTML.exists() or not REPORT_JSON.exists():
        raise RuntimeError(f"Final benchmark exited with {result.returncode} without complete report files.")
...
def main():
    ...
    print("=== DIALFORGE FINAL COLAB ACCEPTANCE BOOTSTRAP v1 ===", flush=True)
    ...
    if args.validate_only:
        print("Final bootstrap validation passed.", flush=True)
        return 0
    code = run_benchmark(python)

    print("\n=== DIALFORGE FINAL ACCEPTANCE COMPLETE ===", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print("\nFinal acceptance interrupted by user.", flush=True)
        raise
    except Exception as exc:
        print("\n=== FINAL ACCEPTANCE STOPPED SAFELY ===", flush=True)
