...
_TEMPORAL_CALL_NEGATION = re.compile(
    r"\b(?:do\s*not|don't|dont)\s+call(?:\s+me|\s+us)?\s+(?:today|tomorrow|tonight|this\s+(?:morning|afternoon|evening|week)|next\s+week|before\b|after\b|until\b)",
    re.I,
)
_THIRD_PARTY_CONTACT_NEGATION = re.compile(
    r"\b(?:do\s*not|don't|dont)\s+(?:call|contact)\s+(?:the\s+)?(?:owner|manager|office|dispatcher|my\s+boss)\b",
    re.I,
)
_WRONG = re.compile(
    r"\bwrong\s+(?:number|person|business|company)\b|\b(?:you(?:'ve| have)\s+got|you\s+have)\s+the\s+wrong\s+(?:number|person|business|company)\b",
    re.I,
)
_VOICEMAIL = re.compile(
    ...
    r"\bvoicemail\b|\banswering\s+machine\b",
    re.I,
)
_INFO = re.compile(
    ...
    r"\b(?:follow\s*up|follow-up)\s+(?:by|via)\s+(?:email|text|phone)\b",
    re.I,
)
_CLEAR_REFUSAL = re.compile(
    ...
    r"\bdon't\s+want\s+(?:a\s+)?meeting\b|\bdo\s+not\s+want\s+(?:a\s+)?meeting\b",
    re.I,
)
_BOOKING_NEGATION = re.compile(
    ...
    r"\bjust\s+checking\s+availability\b|\bchecking\s+availability\b",
    re.I,
)
_BOOKING_CONSENT = re.compile(
    r"\b(?:book|schedule)\b|\bgo\s+ahead\b|\blet(?:'s| us)\s+do\s+it\b|"
    r"\bworks\s+for\s+me\b|\bthat\s+works\b|\bworks\.?\s*(?:book|schedule)?\b",
    re.I,
)
...
_VAGUE_SCHEDULING_SIGNAL = re.compile(
    rf"\b{_WEEKDAY}\b.{{0,50}}\b{_DAYPART}\b.{{0,80}}\b(?:work|works|available|free|diagnostic|meeting|appointment|callback|call)\b|"
    rf"\b(?:today|tomorrow|tonight|next\s+week)\b.{{0,50}}\b{_DAYPART}\b.{{0,80}}\b(?:work|works|available|free|diagnostic|meeting|appointment|callback|call)\b|"
    rf"\b{_DAYPART}\b.{{0,50}}\b(?:work|works|available|free)\b.{{0,80}}\b(?:diagnostic|meeting|appointment|callback|call)\b",
    re.I,
)
...
_SCOPE_RELATION = re.compile(
    r"\b(?:include[sd]?|come[sd]?\s+with|add[sd]?|cover[sd]?|provide[sd]?|contain[sd]?|has|have|"
    r"does\s+not\s+include|doesn't\s+include|without)\b",
    re.I,
)
_REVERSE_SCOPE_RELATION = re.compile(
    r"\b(?:included?|provided|covered|available)\s+(?:with|in|on)\s+(?:the\s+)?(starter|basic|growth|premium|entry[-\s]?level)\b",
    re.I,
)
_FEATURE_TERM = re.compile(
    r"\b(?:seo|search\s+engine\s+optimization|pages?|page\s+count|hosting|maintenance|support|"
    r"revisions?|copy|content|google\s+ads?|paid\s+search|tracking|analytics|crm|automation|"
    r"rankings?|local\s+seo|blog|e-?commerce|booking|forms?)\b",
    re.I,
)
_GUARANTEE = re.compile(
    ...
    r"\b(?:rankings?|leads?|calls?|jobs?|revenue|results?|roi|savings?)\b.{0,90}\b(?:guarantee[sd]?|guaranteed|promise[sd]?)\b",
    re.I,
)
_UNCERTAINTY = re.compile(
    r"\b(?:can't|cannot|can not|don't|do not|won't|will not|wouldn't|would not|not supplied|not provided|"
    r"don't have|do not have|can't confirm|cannot confirm|don't know|do not know|no guarantee|not guaranteed|"
    r"don't want to guess|do not want to guess)\b",
    re.I,
)
...
_CONTROL_LEAK = re.compile(
    r"\b(?:tool trigger|mandatory tool router|private call control rules|dialforge turn decision card|problem \+ impact|"
    r"prospect asserts|call mark_do_not_call|action priority|sales priority|small-model mode)\b|<tool>|"
    r"\brecord_outcome\s*\{|\bbook_meeting\s*\{",
    re.I,
)
...
def booking_components(text: str) -> dict[str, Any]:
    ...
    result = {
        ...
        "time": clock,
        "timezone": timezone,
        "consent": bool(_BOOKING_CONSENT.search(value)) and not negated,
        "negated": negated,
    }
    ...
    if scheduling:
        ...
        if not missing and b["consent"]:
            return RouteDecision(
                "tool",
                "book_meeting",
                {"title": "Scheduled meeting", "starts_at": b["starts_at"], "timezone": b["timezone"], "notes": "Booked from explicit prospect agreement."},
                reason="exact_booking",
            )
        if missing:
            prompts = {
...
def _package_claim_token(sentence: str) -> str | None:
    ...
    for match in _PACKAGE_TOKEN.finditer(value):
        if _SCOPE_RELATION.search(value[match.end():match.end() + 64]):
...
def _scope_relation_supported(sentence: str, source: str) -> bool:
    package = _package_claim_token(sentence)
    features = {m.group(0).lower() for m in _FEATURE_TERM.finditer(sentence)}
    if not package or not features:
        return True
...
def _sanitize_sentence(sentence: str, source: str) -> str:
    ...
    package_claim = _package_claim_token(value)
    scope_claim = bool(package_claim and _FEATURE_TERM.search(value))
    ...
    leak = bool(_CONTROL_LEAK.search(value))

    if leak:
        if scope_claim and guarantee_claim and not _positive_source_supports_guarantee(source):
            return "I can't confirm those package features, and I can't guarantee that outcome."
        if scope_claim and not _scope_relation_supported(value, source):
            return "I don't have the exact package feature breakdown, so I don't want to guess."
        if guarantee_claim and not _positive_source_supports_guarantee(source):
...
def sanitize_marketing_claim(text: str, source: str) -> str:
    ...
    for sentence in _sentences(text):
        ...
        if safe and (not output or output[-1] != safe):
            output.append(safe)
