#!/usr/bin/env python3
"""Dedicated Dialforge Sales Acceptance v2 Colab bootstrap.

This wrapper reuses the proven Final Acceptance v3 environment/bootstrap code
without source-text rewriting. It overrides only the sales runner and report
paths, then delegates to the proven bootstrap implementation.
"""
...
def api_json(url: str):
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "Dialforge-Sales-v2-Bootstrap", "Accept": "application/vnd.github+json"},
    )
...
def main() -> int:
    sha = pinned_sha()
    print("=== DIALFORGE SALES COLAB ACCEPTANCE v2.1 ===", flush=True)
    print("Pinned source SHA:", sha, flush=True)

    base = load_base(sha)

    # Override only the workload/report contract. All Python/CUDA/Ollama setup,
    # validation, caching and model execution behavior stays on the proven path.
    base.RUNNER = ROOT / "dialforge_sales_acceptance.py"
    ...
    base.RUNNER_PATH = "benchmarks/dialforge_sales_acceptance.py"

    print("Sales runner:", base.RUNNER_PATH, flush=True)
    print("Report directory:", base.REPORT_DIR, flush=True)

    # The base bootstrap resolves this environment variable too. Keep it pinned
    # so bootstrap + runner always come from exactly the same immutable commit.
    ...
    print("\n=== DIALFORGE SALES ACCEPTANCE v2.1 COMPLETE ===", flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print("\n=== SALES ACCEPTANCE STOPPED SAFELY ===", flush=True)
