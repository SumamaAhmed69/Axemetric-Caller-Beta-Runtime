#!/usr/bin/env python3
"""Dialforge consultative sales acceptance benchmark v2.

Tests the local caller as a salesperson rather than a generic chatbot. V2 mirrors the
production sales/guard rules more closely, preserves punctuation while length-limiting,
and explicitly tests factual package scope, CTA duration, closing discipline and action
integrity.
...
PACKAGE SCOPE: Exact feature/inclusion differences between Starter, Growth and Premium are NOT supplied. Plan names and prices do not imply features.
...
PROHIBITED CLAIMS: no guaranteed rankings, leads, revenue, savings, exclusivity, scarcity, fake deadlines, fake audits, invented case studies, invented package inclusions, or invented research about the prospect.
""".strip()
...
2. DISCOVER BEFORE PRESCRIBING: ask one useful question at a time. Prefer questions that reveal current process, friction, impact, priorities, provider situation, decision process, or timing. If a prospect states a problem but its impact is still unclear, ask one concise impact question before pitching unless they directly ask what you do or what the next step is.
3. LISTEN AND LABEL: reflect only the important part of what the prospect just said. Use their language rather than generic marketing jargon.
...
5. VALUE BRIDGE: connect only the relevant supplied capability to the problem the prospect actually acknowledged. Never dump every feature.
...
7. CLOSE WHEN EARNED: if there is genuine relevance, ask clearly for the configured next step. If the prospect asks what the next step is or says they are open to the diagnostic, ask for an exact day and time. Do not promise to send a booking link or merely say the request was noted.
...
- BUSY: compress immediately. Give the reason in one sentence. Do not change the configured 15-minute meeting into an invented 10-minute meeting; either ask one quick question or ask for a better time.
...
- PRICE: answer directly from supplied pricing. Never hide a known price behind a meeting. Package names/prices do not imply package features. If exact package differences are not supplied, say you do not have the exact breakdown rather than inventing it.
- SEND INFO: record a follow-up request with the tool when appropriate. Never pretend an email, text, booking link, or file was sent.
- SKEPTICAL / BAD AGENCY EXPERIENCE: validate the specific concern, avoid hype, use only supplied proof, understand what failed before proposing another meeting unless the prospect directly asks for a next step.
...
VOICE BEHAVIOR
Usually speak in one or two short sentences, maximum three. Prefer roughly 8-25 words and stay under 34 where possible. Ask at most one question per turn. Use contractions and natural language. Avoid buzzword stacks, canned enthusiasm, fake familiarity, excessive compliments, and repeating the prospect's name.
Do not sound submissive or apologetic for calling, but respect boundaries immediately.

INTEGRITY
Use only CAMPAIGN DOSSIER and supplied lead/conversation facts as ground truth. Never invent research, package inclusions, pricing, results, urgency, proof, consent, availability, actions or bookings. Never pretend to be human. If asked, say you are an automated calling assistant.
...
CONVERSATIONS = [
    ...
    },
    {
        ...
        "turns":[
            ...
            {"prospect":"The last company promised a ton of leads and it was a waste of money.","expect":"skepticism_after_bad_provider"},
...
ACTION_RE = re.compile(r"\b(?:i|we)(?:(?:['’]\s*|\s+)(?:ll|will|ve|have))\s+(?:send|sent|email|emailed|text|texted|message|messaged|call|called|follow\s+up)\b", re.I)
CALL_ACTION_RE = re.compile(r"\b(?:i|we)(?:(?:['’]\s*|\s+)(?:ll|will))\s+(?:call|call\s+back)\b", re.I)
...
DURATION_RE = re.compile(r"\b(\d{1,3})\s*(?:-|\s)?minute\b", re.I)
INVENTED_SCOPE_RE = re.compile(r"\b(?:starter|growth|premium|\$750|\$1,?200|\$2,?000).{0,80}\b(?:includes?|adds?|comes with|covers?|gives you|provides?)\b", re.I)
...
def truncate_words_preserve_punctuation(text:str,limit:int)->str:
    tokens=str(text or "").split()
    kept=[]; count=0
    for token in tokens:
        token_words=max(1,len(words(token)))
        if count+token_words>limit: break
        kept.append(token); count+=token_words
...
def clean_spoken(text:str)->tuple[str,list[str]]:
    ...
    for part in parts:
        ...
        if ACTION_RE.search(part):
            ...
            part="What exact day and time works best?" if CALL_ACTION_RE.search(part) else "I can note that request for the team."
        if part and (not out or part!=out[-1]): out.append(part)
        if len(out)>=3: break
    value=" ".join(out)
    if len(words(value))>MAX_WORDS:
        value=truncate_words_preserve_punctuation(value,MAX_WORDS)
...
def duration_ok(text:str)->bool:
    return all(int(m.group(1))==15 for m in DURATION_RE.finditer(text))
...
def expectation_score(expect:str,text:str)->tuple[float,list[str]]:
    ...
    base=[
        ...
        ("one_question",one_question(text)),
        ("no_meta",not META_RE.search(text)),
        ("no_pressure",not PRESSURE_RE.search(text)),
        ("grounded_price",price_ok(text)),
        ("grounded_duration",duration_ok(text)),
    ]
    checks.extend(base)
    if expect=="identity":
        checks += [("identity",any(x in lower for x in ["automated","assistant","northstar","dialforge"])),("purpose",any(x in lower for x in ["diagnostic","digital","website","growth"]))]
    elif expect=="busy":
        checks += [("brief_busy",len(words(text))<=28)]
    elif expect=="discovery":
        checks += [("asks_discovery","?" in text)]
    elif expect=="bridge":
        checks += [("reflects_problem",any(x in lower for x in ["miss","call","follow","lead","job"])),("asks_or_bridges",("?" in text) or any(x in lower for x in ["capture","follow-up","diagnostic"]))]
    elif expect=="relevant_value":
        checks += [("relevant_offer",any(x in lower for x in ["diagnostic","website","tracking","visibility","conversion","missed","follow-up"]))]
    elif expect=="close":
        checks += [
            ("clear_next_step",any(x in lower for x in ["15-minute","15 minute","diagnostic","book","schedule","day and time","what time","time works"])),
            ("scheduling_question","?" in text and any(x in lower for x in ["day","time","when","schedule","works best"])),
        ]
    elif expect=="provider_objection":
        checks += [("no_provider_attack",not ATTACK_RE.search(text)),("ack_or_question",("?" in text) or any(x in lower for x in ["understand","makes sense","fair","got it"]))]
    elif expect=="impact":
        checks += [("impact_question",("?" in text) and any(x in lower for x in ["lead","quality","week","cost","impact","job","close","revenue","booked"]))]
    elif expect=="no_switch_pressure":
        checks += [("no_switch_attack",not ATTACK_RE.search(text)),("allows_existing",any(x in lower for x in ["don't need to switch","without switching","keep","existing","diagnostic","second look","no need to change"]))]
    elif expect=="price":
        checks += [("direct_price",any(x in text for x in ["$750","$1,200","$2,000"]))]
    elif expect=="scope_unknown":
        acknowledges_unknown=any(x in lower for x in ["don't have the exact","do not have the exact","exact breakdown","not supplied","don't want to guess","do not want to guess","can't give you the exact","cannot give you the exact"])
        checks += [("does_not_invent_scope",not INVENTED_SCOPE_RE.search(text) and acknowledges_unknown)]
    elif expect=="skepticism":
        checks += [("no_hype",not any(x in lower for x in ["best agency","transform your business","skyrocket","guaranteed"])),("acknowledges",any(x in lower for x in ["fair","understand","makes sense","reasonable","frustrating","skeptical","concern","hear you","i hear"])),]
    elif expect=="skepticism_after_bad_provider":
        checks += [
            ("acknowledges",any(x in lower for x in ["fair","understand","frustrating","hear you","waste","promise"])),
            ("does_not_jump_to_send",not ACTION_RE.search(text)),
            ("trust_discovery","?" in text or any(x in lower for x in ["what went wrong","what failed","what would you need","what would make","what did they"])),
        ]
    elif expect=="no_guarantee":
        checks += [("rejects_guarantee",any(x in lower for x in ["can't guarantee","cannot guarantee","don't guarantee","no guarantee","wouldn't promise"]))]
    passed=sum(1 for _,ok in checks if ok)
    return (100.0*passed/max(1,len(checks)),[name for name,ok in checks if not ok])
...
def chat(model:str,messages:list[dict[str,Any]],tools=None)->dict[str,Any]:
    payload={"model":model,"messages":messages,"stream":False,"think":False,"keep_alive":"30m","options":{"temperature":0.15,"top_p":0.8,"num_predict":80}}
...
def run_tools(model:str)->dict[str,Any]:
    ...
    for case in TOOL_CASES:
        ...
        if case["id"]=="vague_callback":
            spoken,_=clean_spoken(result["content"]); ok=ok and ("?" in spoken or any(x in spoken.lower() for x in ["what time","specific time","exact time","day and time"]))
...
def main()->int:
    ...
    print("=== DIALFORGE CONSULTATIVE SALES ACCEPTANCE v2 ===",flush=True)
    results={}
    ...
    report={"version":"sales-v2","timestamp_utc":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"winner":winner,"models":results,"speech":speech,"hardware":{"platform":platform.platform(),"python":platform.python_version(),"ram_gb":round(psutil.virtual_memory().total/1024**3,2),"torch":torch.__version__,"cuda":torch.version.cuda,"gpu":torch.cuda.get_device_name(0) if torch.cuda.is_available() else None}}
